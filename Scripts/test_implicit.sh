...
#
#SBATCH --mem-per-cpu=750 # megabytes
...
parameter_file=sriLanka_implicit_result.txt
map_file=sriLanka.dat
TIMESTAMP=`date +%Y-%m-%d_%H-%M-%S`
output_filename="$serverPath/Results/statistics_implicit_result_$SLURM_NTASKS-$SLURM_CPUS_PER_TASK-on-$TIMESTAMP"
